...
blue_df_jg = df[(df.TEAMID == 100) & (df.TEAMPOSITION == 'JUNGLE')].rename(columns={'CHAMPIONNAME': 'j_champ'})
blue_df_mid = df[(df.TEAMID == 100) & (df.TEAMPOSITION == 'MIDDLE')].rename(columns={'CHAMPIONNAME': 'm_champ'})
blue_df_mg = pd.merge(blue_df_mid, blue_df_jg, on=['GAMEID'])

red_df_jg = df[(df.TEAMID == 200) & (df.TEAMPOSITION == 'JUNGLE')].rename(columns={'CHAMPIONNAME': 'j_champ'})
red_df_mid = df[(df.TEAMID == 200) & (df.TEAMPOSITION == 'MIDDLE')].rename(columns={'CHAMPIONNAME': 'm_champ'})
red_df_mg = pd.merge(red_df_mid, red_df_jg, on=['GAMEID'])
...
tmp = tmp[['m_champ',  'j_champ', 'win_x_x', 'enemy_m_champ', 'enemy_j_champ']]
